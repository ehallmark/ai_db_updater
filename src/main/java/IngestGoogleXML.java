...
public class IngestGoogleXML {
    ...
    private static final String GS_BUCKET = "gs://machine_learning_cloud_data/";
    private static final String bucketToSaveIn = "spark-patents";
    ...
    public static void main(String[] args) {
        try {
            ...
            sparkConf.setAppName("Ingest Google XML");
            SparkSession spark = SparkSession.builder()
                    .appName("Ingest Google XML")
                    .getOrCreate();
            ...
            while(lastIngestedDate<=endDateInt) {
                ...
                allDates.add(finalLastIngestedDate);
            }

            spark.createDataset(allDates,Encoders.INT()).flatMap(finalLastIngestedDate->{
                List<SparkPatent> data = new ArrayList<>();
                try {
                    try {
                        String dateStr = String.format("%06d", finalLastIngestedDate);
                        String url = base_url + "/20" + dateStr.substring(0, 2) + "/ipg" + String.format("%06d", finalLastIngestedDate) + ".zip";
                        ...
                        while ((line = br.readLine()) != null) {
                            if (line.contains("<?xml") && !firstLine) {
                                ...
                                if (handler.getPatentNumber() != null && !handler.getFullDocuments().isEmpty()) {
                                    final String assignee = handler.getAssignees().size()>0 ? handler.getAssignees().iterator().next() : null;
                                    handler.getFullDocuments().forEach(doc->{
                                        data.add(new SparkPatent(handler.getPatentNumber(),assignee, String.join(" ", doc)));
                                    });
                                }
                ...
                if(data.isEmpty()) return Collections.emptyIterator();
                return data.iterator();

            },Encoders.bean(SparkPatent.class)).write()
                    .mode(SaveMode.Append)
                    .format(AVRO_FORMAT)
                    .save(GS_BUCKET+bucketToSaveIn);

            Database.updateLastIngestedDate(endDateInt);
